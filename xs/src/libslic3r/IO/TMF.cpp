...
{
	std::cout<< "Just started TMFEditor::read_model()" << std::endl;
...
{
	std::cout<< "Just started TMFEditor::produce_TMF()" << std::endl;
...
{
	std::cout<< "Just started TMFEditor::consume_TMF()" << std::endl;
...
TMFParserContext::startElement(const char *name, const char **atts)
{
	...
	switch (m_path.size()){
	case 0:
		std::cout<< "TMFParserContext case 0 got hit." << std::endl;
		// Must be <model> tag.
